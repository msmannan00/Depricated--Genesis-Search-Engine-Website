...
class webindexer_model extends Model
{
    ...
    {
        $query = "SELECT document_id FROM cache_word cw,cache_nodes cn ON cw.word_id=cn.word_id";
    ...
    public function readDocument()
    {
        ...
        if (1!=1 && Cache::has('index_model') && 1!=1)
        {
        ...
        else
        {
            ...
            $query = "SELECT id,keyword FROM webpages GROUP BY title";
            $result = DB::select($query);
            foreach($result as $row)
            {
                $doc_id = $row->id;
    ...
    public function updateCacheModel()
    {
        ...
        $word_list = 'INSERT INTO `cache_word`(`word_id`,`word`,`inverse_document_frequency`) VALUES ';
        $document_list = 'INSERT INTO `cache_nodes`(`document_id`,`word_id`,`term_frequency`) VALUES ';

        $keys = array_keys($local_web_indexer_model->word_list);

        $auto_counter = 0;
        $size_keys = sizeof($keys);
        for($counter1 = 0; $counter1 < $size_keys; $counter1 ++)
        {
            $dict = $local_web_indexer_model->word_list[$keys[$counter1]];
            $word_info = $dict->cache_nodes_list;
            $word_list .= "(".$auto_counter.",'".$keys[$counter1]."',".round($dict->inverse_document_frequency,2)."),";

            $size_word = sizeof($word_info);
            for($counter2 = 0; $counter2 < $size_word; $counter2 ++)
            {
                $term_frequ = $word_info[$counter2]->term_frequency;
                $doc_id = $word_info[$counter2]->document_id;
                $document_list .= "('".$doc_id."',".$auto_counter.",".round($term_frequ,3)."),";
            }
            //echo $keys[$counter1]." : ".$local_web_indexer_model->word_list[$keys[$counter1]]->inverse_document_frequency;
            $auto_counter+=1;
        }

        //echo substr($document_list,0,strlen($document_list)-1);

        DB::insert(substr($document_list,0,strlen($document_list)-1));
        DB::insert(substr($word_list,0,strlen($word_list)-1));
